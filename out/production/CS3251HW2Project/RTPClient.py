...
class RTPClient:
	...
	def connect(self):
		...
		pikmin.getHeader().setseqNum(1)
		#Incase server crashes initially during 3-way handshake
		# try:
			# sock.sendto(bytes(pikmin),(self.host,int(self.portNum)))
			# resSock = sockRecv.recvfrom(4096)
		# except Exception as e:
			# print(str(e))
			# print("Socket timed out. Server may have crashed.")
			# return
		sock.sendto(pikmin.getHeader().getByteArray() + bytes(pikmin.getData()), (self.host, int(self.portNum)))
		resSock = sockRecv.recvfrom(4096)
		tfinal = time.time()
	...
	def checksum(self, aPacket): # dude use CRC32. or not. we just need to decide on one
		...
		first = hashlib.md5(bytes(str(aPacket.getData()), 'utf-8')).hexdigest()
